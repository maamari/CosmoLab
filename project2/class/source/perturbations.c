...
int perturb_derivs(double tau,
  ...
  /* for use with dmeff */
  double R_dmeff, beta_dmeff, beta=0., slip;
  ...
  if (_scalars_) {
    ...
    if (pba->has_dmeff == _TRUE_) {
      ...
      if (ppt->gauge == newtonian) {
        dy[pv->index_pt_delta_dmeff] = -(y[pv->index_pt_theta_dmeff]+metric_continuity); /* dmeff density */

        dy[pv->index_pt_theta_dmeff] = - a_prime_over_a*y[pv->index_pt_theta_dmeff] + metric_euler; /* dmeff velocity */
      }
      ...
      if (ppt->gauge == synchronous) {
        ...
        dy[pv->index_pt_theta_dmeff] =
          - a_prime_over_a*y[pv->index_pt_theta_dmeff] + metric_euler
          + k2*pvecback[pba->index_bg_cdmeff2]*(y[pv->index_pt_delta_dmeff])
          + pvecback[pba->index_bg_dkappa_dmeff]*(theta_b - y[pv->index_pt_theta_dmeff]); /* dmeff velocity */
      }

      beta_dmeff = (pvecback[pba->index_bg_dkappa_dmeff]/pvecthermo[pth->index_th_dkappa]) * R_dmeff / (1. + R);
    }
    ...
    if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
      ...
      if (pba->has_dmeff == _TRUE_){
        dy[pv->index_pt_theta_b] += R_dmeff*pvecback[pba->index_bg_dkappa_dmeff]*(y[pv->index_pt_theta_dmeff]-theta_b);
      }
      ...
      if(pba->has_dmeff==_TRUE_){
        beta += beta_dmeff;
        slip += beta_dmeff*(dy[pv->index_pt_theta_dmeff]-a_prime_over_a/2.*(y[pv->index_pt_theta_dmeff]-theta_b));
      }
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      /** - ----> if photon tight-coupling is on: */
        ...
        if (pba->has_dmeff == _TRUE_)
          dy[pv->index_pt_theta_g] += pvecback[pba->index_bg_dkappa_dmeff]/R * (y[pv->index_pt_theta_dmeff]-theta_b);
