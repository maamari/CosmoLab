...
int thermodynamics_init(
  ...
  /** - --> fill thermodynamics_table with dmeff quantities */
  if (pba->has_dmeff == _TRUE_){
    ...
    for (index_tau=0; index_tau < pth->tt_size; index_tau++) {
      ...
      pth->thermodynamics_table[index_tau*pth->th_size+pth->index_th_Tdmeff] = pvecback[pba->index_bg_Tdmeff];
      pth->thermodynamics_table[index_tau*pth->th_size+pth->index_th_dkappa_dmeff] = pvecback[pba->index_bg_dkappa_dmeff];
      pth->thermodynamics_table[index_tau*pth->th_size+pth->index_th_dkappaT_dmeff] = pvecback[pba->index_bg_dkappaT_dmeff];
      pth->thermodynamics_table[index_tau*pth->th_size+pth->index_th_cdmeff2] = pvecback[pba->index_bg_cdmeff2];
    }
...
int thermodynamics_dmeff_init(struct background *pba,
  ...
  class_alloc(pdmeff->thermo_Tb,  pth->tt_size*sizeof(double),pth->error_message);
  class_alloc(pdmeff->thermo_ddTb,pth->tt_size*sizeof(double),pth->error_message);
...
int thermodynamics_dmeff_free(struct dmeff_thermo *pdmeff){
  /* Allocated in thermodynamics_dmeff_init(). */
  free(pdmeff->thermo_Tb);
  free(pdmeff->thermo_ddTb);
 ...
 * @param pba        Input:        background structure
 * @param pth        Input:        thermodynamics structure
 * @param pdmeff     Input:        dmeff_thermo structure
...
int thermodynamics_dmeff_rate(struct background *pba,
  ...
  double z, a, H, rho_baryon, Tb, Tdmeff, dTdtau_T;
  double cn, rate, avg_term;
  int last_index;
  ...
  z = 1./a - 1.;
  rho_baryon = pvecback[pba->index_bg_rho_b];

  /* Extract dmeff temperature. */
  if(pba->sigma_dmeff == 0.0){
    pvecthermo[pth->index_th_Tdmeff] = 0.0;
  }
  Tdmeff = pvecthermo[pth->index_th_Tdmeff];

  /* Extract baryon temperature. */
  if (z > pth->z_table[pth->tt_size-1]){
    Tb = pba->T_cmb*(1.+z);
  } else{
  ...
  pvecthermo[pth->index_th_Tb] = Tb;

  /* Initialize rates to save in pvecthermo. */
  pvecthermo[pth->index_th_dkappa_dmeff]  = 0.0;
  pvecthermo[pth->index_th_dkappaT_dmeff] = 0.0;

  // convert baryon density (which includes He) into units of kg / m^2 / Mpc
  rho_baryon *= 3.*_c_*_c_/(8.*_PI_*_G_*_Mpc_over_m_);

  cn = pow(2.,(pba->npow_dmeff + 5.)/2.) * tgamma(3. + pba->npow_dmeff / 2.) / (3.*_SQRT_PI_);

  rate = a * (1.-pth->YHe) * rho_baryon * cn * pba->sigma_dmeff/(pba->m_dmeff + _m_H_);

  /* Include internal degrees of freedom from phase space distribution. */
  rate *= (2.*pba->spin_dmeff + 1.) * (2.*0.5 + 1.); // proton has spin 1/2

  /* Thermal contribution. */
  avg_term = (Tb/_m_H_ + Tdmeff/pba->m_dmeff)*_k_B_/(_c_*_c_);
  rate *= pow(avg_term, (pba->npow_dmeff + 1.)/2.);

  /* Save rates */
  pvecthermo[pth->index_th_dkappa_dmeff]  += rate;
  pvecthermo[pth->index_th_dkappaT_dmeff] += rate * pba->m_dmeff / (pba->m_dmeff + _m_H_);
...
int thermodynamics_dmeff_derivs(double tau,
  ...
  /* Find energy exchange rate. */
  pvecthermo[pth->index_th_Tdmeff] = y[pdmeff->index_ti_Tdm];
  class_call(thermodynamics_dmeff_rate(pba,pth,pdmeff,pvecback,pvecthermo),
             pth->error_message,
             pth->error_message);
  ...
  if (z > pdmeff->zdec){
    ...
    dy[pdmeff->index_ti_Tdm] = -2.*a*H*y[pdmeff->index_ti_Tdm]
      + 2.*pvecthermo[pth->index_th_dkappaT_dmeff] * (pvecthermo[pth->index_th_Tb] - y[pdmeff->index_ti_Tdm]);
  }
 ...
 *  It relies on baryon temperature in thermodynamics_table.
 *
...
int thermodynamics_dmeff_temperature(struct precision *ppr,
  ...
  /* Set baryon temperature from thermodynamics_table and create its 2nd derivative. */
  for (index_tau=0; index_tau < pth->tt_size; index_tau++){
    pdmeff->thermo_Tb[index_tau] = pth->thermodynamics_table[index_tau*pth->th_size+pth->index_th_Tb];
  }
  class_call(array_spline_table_lines(pth->z_table,
                                      pth->tt_size,
                                      pdmeff->thermo_Tb,
  ...
  a = pba->background_table[0*pba->bg_size+pba->index_bg_a];
